...
export function dinnersReducer(
  state: DinnersState = INITIAL_STATE,
  action: any
) {
  switch (action.type) {
    ...
    case DinnersActions.REMOVE_DINNER:
      return tassign(state, {
        dinners: state.dinners.filter(dinner => dinner.id !== action.payload)
      });
    case DinnersActions.UPDATE_DINNER:
      return tassign(state, {
        dinners: state.dinners.map(
          dinner => (dinner.id === action.payload.id ? action.payload : dinner)
        )
      });
      ...
      return tassign(state, {
        dinners: state.dinners.map(dinner => {
          if (dinner.id === action.payload.dinnerId) {
            return tassign(dinner, {
              attendees: [...dinner.attendees, action.payload.attendee.id]
            });
          }
          return dinner;
        })
